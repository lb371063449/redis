...
public class AccessLogKafkaSpout extends BaseRichSpout {
    ...
    private void startKafkaConsumer() {
        ...
        props.put("zookeeper.connect", "192.168.208.129:2181,192.168.208.130:2181,192.168.208.131:2181");
    ...
    private class KafkaMessageProcessor implements Runnable {
        ...
        public void run() {
            ...
            while (it.hasNext()) {
                ...
                log.debug("【AccessLogKafkaSpout中的Kafka消费者接收到一条日志】message= {}", message);
                try {
                    queue.put(message);
    ...
    public void nextTuple() {
        if (queue.size() > 0) {
            try {
                ...
                log.debug("【AccessLogKafkaSpout发射出去一条日志】message= {}", message);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
